...
AppMenuButtonRightClickMenu.prototype = {
  ...
  _init: function _init(parent, actor) {
    ...
    if (monitors.length > 1) {
      for (var i = 0; i < monitors.length; i++) {
        var itemChangeMonitor = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_move_monitor% %d').format(i + 1), 'view-fullscreen');
    ...
    this.itemCloseAllWindow = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_close_all%'), 'window-close');
    this.itemCloseAllWindow.connect('activate', Lang.bind(this, this._onCloseAllActivate));

    this.itemCloseWindow = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_close%'), 'window-close');
    this.itemCloseWindow.connect('activate', Lang.bind(this, this._onCloseWindowActivate));

    this.itemMinimizeWindow = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_minimize%'));
    this.itemMinimizeWindow.connect('activate', Lang.bind(this, this._onMinimizeWindowActivate));

    this.itemMaximizeWindow = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_maximize%'));
    this.itemMaximizeWindow.connect('activate', Lang.bind(this, this._onMaximizeWindowActivate));

    this.itemMoveToLeftWorkspace = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_move_left%'), 'back');
    this.itemMoveToLeftWorkspace.connect('activate', Lang.bind(this, this._onMoveToLeftWorkspace));

    this.itemMoveToRightWorkspace = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_move_right%'), 'next');
    this.itemMoveToRightWorkspace.connect('activate', Lang.bind(this, this._onMoveToRightWorkspace));

    this.itemOnAllWorkspaces = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_visible_workspace%'), 'edit-copy');
    ...
    if (this._applet.showPinned != FavType.none) {
      if (this.isFav) {
        this.itemtoggleFav = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_unpin%'), 'remove');
        this.itemtoggleFav.connect('activate', Lang.bind(this, this._toggleFav));
      } else {
        this.itemtoggleFav = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_pin%'), 'bookmark-new');
        this.itemtoggleFav.connect('activate', Lang.bind(this, this._toggleFav));
      }
    }
    if (this.isFavapp) {
  ...
  _settingsMenu: function _settingsMenu() {
    this.subMenuItem = new SpecialMenuItems.SubMenuItem(this, _('%ctx_settings%'));
    var subMenu = this.subMenuItem.menu;

    this.reArrange = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_rearrange%'), this._applet.arrangePinned);
    ...
    this.showPinned = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_show_pinned%'), this._applet.showPinned);
    ...
    this.showThumbs = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_show_thumbs%'), this._applet.showThumbs);
    ...
    this.stackThumbs = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_stack_thumbs%'), this._applet.stackThumbs);
    ...
    this.enablePeek = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_hover_peek%'), this._applet.enablePeek);
    ...
    this.showRecent = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_show_recent%'), this._applet.showRecent);
    ...
    this.verticalThumbs = new SpecialMenuItems.SwitchMenuItem(this, _('%ctx_vertical_thumbs%'), this._applet.verticalThumbs);
    ...
    this.settingItem = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_go_settings%'));
  ...
  addSpecialItems: function addSpecialItems() {
    ...
    if (this.app.get_id() == 'nemo.desktop' || this.app.get_id() == 'nemo-home.desktop') {
      ...
      if (historys === null) {
        var install = new SpecialMenuItems.IconNameMenuItem(this, _('%ctx_install_gda%'));
        ...
        try {
          ...
          for (var _i3 = 0; _i3 < historys.length; _i3++) {
            var history = historys[_i3];
  ...
  _onToggled: function _onToggled(actor, event) {
    ...
    if (this.metaWindow.is_on_all_workspaces()) {
      this.itemOnAllWorkspaces.label.text = _('%ctx_on_this_workspace%');
      this.itemMoveToLeftWorkspace.actor.hide();
      this.itemMoveToRightWorkspace.actor.hide();
    } else {
      this.itemOnAllWorkspaces.label.text = _('%ctx_visible_workspace%');
    ...
    if (this.metaWindow.get_maximized()) {
      this.itemMaximizeWindow.label.text = _('%ctx_unmaximize%');
    } else {
      this.itemMaximizeWindow.label.text = _('%ctx_maximize%');
    }
    if (this.metaWindow.minimized) {
      this.itemMinimizeWindow.label.text = _('%ctx_restore%');
    } else {
      this.itemMinimizeWindow.label.text = _('%ctx_minimize%');
    }
  },
...
PopupMenuAppSwitcherItem.prototype = {
  ...
  getMetaWindows: function getMetaWindows() {
    ...
    if (this.app.wmClass && this.metaWorkspace) {
      windows = this.metaWorkspace.list_windows().filter(function (win) {
        return _this2.app.wmClass == win.get_wm_class_instance();
      }).reverse();
    } else {
      windows = this.app.get_windows().filter(function (win) {
        // var isDifferent = (win != this.metaWindow)
        var isSameWorkspace = win.get_workspace() == _this2.metaWorkspace && Main.isInteresting(win);
  ...
  refreshRows: function refreshRows() {
    ...
    if (appContLength2 < this.thumbnailsSpace && appContLength3 > 0) {
      ...
      for (var _i6 = 0; _i6 < _thumbsToMove; _i6++) {
        var _actor = _children[_i6] ? _children[_i6] : null;
        if (_actor === null) {
          break;
        }
  ...
  _init: function _init(parent, metaWindow) {
    ...
    if (this.metaWindow) {
      this.metaWindow.connect('notify::title', function () {
        _this3._label.text = _this3.metaWindow.get_title();
      });
    ...
    this.actor.connect('enter-event', function () {
      if (!_this3.isFavapp) {
        var parent = _this3._parent._parentContainer;
        parent.shouldOpen = true;
        parent.shouldClose = false;
        _this3._hoverPeek(_this3._applet.peekOpacity, _this3.metaWindow);
        _this3.actor.add_style_pseudo_class('outlined');
        _this3.actor.add_style_pseudo_class('selected');
        _this3.button.show();
        if (_this3.metaWindow.minimized && _this3._applet.enablePeek) {
          _this3.metaWindow.unminimize();
          if (_this3.metaWindow.is_fullscreen()) {
            _this3.metaWindow.unmaximize(global.get_current_time());
          }
          _this3.wasMinimized = true;
        } else {
          _this3.wasMinimized = false;
        }
      }
    });
